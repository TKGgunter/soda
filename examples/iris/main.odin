...
main :: proc() {
    ...
    head(df)
    calc_distance :: proc(vars: []Data, constants: []Data) -> f64 {
        pl := vars[0].(f64)
    ...
    for species in unique_species.([]string) {
        ...
        species_df, _ := soda.filter(&df, {"species", .Eq, Data(species)})
        ...
        species_distance, _ := soda.calc_to_column(species_df, calc_distance, []string{"sepal_width", "sepal_length", "petal_width", "petal_length"}, []Data{sw_mean, sl_mean, pw_mean, pl_mean})
